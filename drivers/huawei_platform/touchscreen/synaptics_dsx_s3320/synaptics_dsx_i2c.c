...
static int s4322_synaptics_force_update(struct synaptics_rmi4_data *rmi4_data);
...
static int s4322_set_glove_mode(struct synaptics_rmi4_data *rmi4_data, unsigned long value)
{
	...
	ret = s4322_synaptics_force_update(rmi4_data);
	if (ret < 0) {
		tp_log_err("%s: Failed to force s4322 update!ret=%d\n", __func__, ret);
		return ret;
	}
	return 0;
}
...
static int s4322_set_effective_window(struct synaptics_rmi4_data *rmi4_data,struct holster_mode *effective_window_info)
{
	...
	mutex_lock(&rmi4_data->rmi4_window_mutex);
	ret = synaptics_rmi4_i2c_read(rmi4_data,
			rmi4_data->f12_ctrl_base_addr + F12_2D_CTRL25_TD4300,
			&f12_2d_ctrl25[0],
...
{
	int ret;
	int offset = F12_2D_CTRL26;

	unsigned char data = enable;
	unsigned char *id_str = rmi4_data->rmi4_mod_info.product_id_string;
	if (0 == strncasecmp(id_str,PID_BOE_CANN, PID_BOE_CANN_LEN) ||
		0 == strncasecmp(id_str,PID_TM_CANN, PID_TM_CANN_LEN) ||
		0 == strncasecmp(id_str,PID_LG_CANN, PID_LG_CANN_LEN))
		offset = F12_2D_CTRL26_TD4300;
	if (enable)
	{
		ret = set_effective_window(rmi4_data,&synap_holster_info);
		if (ret < 0)
...
static int synaptics_read_roi_switch(struct synaptics_rmi4_data *rmi4_data)
{
	...
	roi_ctrl_addr = rmi4_data->f51_ctrl_base_addr + F51_CUSTOM_CTRL03_ROI_Control;
	roi_control_bit = 0;
	...
	rmi4_data->roi_enabled = roi_switch;
				   ...
				   const char *buf, size_t count)
{
	...
	if (rmi4_data->roi_enabled == value) {
...
static int add_easy_wakeup_interfaces(struct device *dev)
{
	...
	if((true == rmi4_data->board->roi_supported) && (SYNAPTICS_S3320 == rmi4_data->board->ic_type))
	{
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (f51_roi_switch) {
		if ((temp_finger_status != pre_finger_status)
			 ...
			 temp_finger_status)) {
			mutex_lock(&rmi4_data->rmi4_roi_mutex);
			roi_data_addr = rmi4_data->f51_data_base_addr + F51_CUSTOM_DATA56_ROI_Data;
...
static void synaptics_wake_recovery(struct synaptics_rmi4_data *rmi4_data)
{
	...
	*/
	...
	retval = synaptics_set_roi_switch(rmi4_data->roi_enabled, rmi4_data);
	if (retval < 0) {
