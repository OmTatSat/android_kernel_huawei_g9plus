...
static int ilitek_i2c_write_update(struct i2c_client *client, uint8_t * cmd, int length)
{
	int ret;
	struct i2c_msg msgs[] = {
...
static int inwrite(unsigned int address)
{
	...
	//tp_log_info("%s, data=0x%x, outbuff[0]=%x, outbuff[1]=%x, outbuff[2]=%x, outbuff[3]=%x\n", __func__, data, outbuff[0], outbuff[1], outbuff[2], outbuff[3]);
	return data;
}
...
int ilitek_upgrade_firmware(void)
{
	...
	//tp_log_info("df_startaddr=0x%lX, df_endaddr=0x%lX, df_checksum=0x%lX\n", df_startaddr, df_endaddr, df_checksum);
	#if 1
	...
	//for (ret = 0; ret < 0x100; ret++) {
		CTPM_FW[ret + 32] = fw->data[ret];
		//tp_log_info("0x%02X,", CTPM_FW[ret + 32]);
		if ((ret + 1) % 16 == 0) {
			//tp_log_info("\n");
		}
...
Retry:
		...
		for(i = ap_startaddr; i < ap_endaddr; i += 0x20) {
			...
			if(ilitek_i2c_write_update(i2c.client, buf, 36) < 0) {
			...
			//tp_log_info("%cILITEK: Firmware Upgrade(Data flash), %02d%c.",0x0D,upgrade_status,'%');
			//tp_log_info("ILITEK: Firmware Upgrade(Data flash), %02d%c. ", upgrade_status, '%');
			mdelay(3);
		}
		buf[0] = 0x1b;
		buf[1] = 0x62;
		...
		tp_log_info("upgrade ILITEK_IOCTL_I2C_RESET end \n");
		buf[0] = 0x10;
		ilitek_i2c_write_update(i2c.client, buf, 1);
		ret = ilitek_i2c_read_update(i2c.client, buf, 3);
